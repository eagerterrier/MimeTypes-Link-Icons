...
//console.log( '# of links found: '+content_links.length );

		if(i18n_mtli.oldwp == 1){
			i18n_mtli.mime_array = i18n_mtli.mime_array.split(',');
		}
//console.log( '# of mime types: '+i18n_mtli.mime_array.length );

		jQuery.each(i18n_mtli.mime_array, function( i, elm ) {
//console.log( 'Evaluating mime_type '+i+' : '+elm );
...
//var filtered = content_links.filter(function( index ) {
			contentLinks.filter(function(){
				return ( jQuery(this).attr('href').search(reg) >= 0 );
			}).addClass('mtli_attachment mtli_'+elm);

//console.log( 'Evaluating mime_type '+i+' : '+elm+"\t\t\t"+'filtered links: '+filtered.length );
		});
	}


	if(i18n_mtli.hidethings == 1){
//console.log( 'nr of elements found : '+jQuery(i18n_mtli.avoid_selector).length );
		jQuery(i18n_mtli.avoid_selector).each(function(){
//console.log( 'testing elm '+index+' : '+elm );
			// If the current element has the mtli_attachment class, remove it
			if( jQuery(this).hasClass('mtli_attachment') ) {
//console.log( "\t"+'element has class mtli_attachment' );
...
//console.log( "\t\t"+'rel mtli_filesize found' );
					jQuery(this).removeAttr('rel');
				}
			}
			// If the current element has descendants with the mtli_attachment class, remove it
//console.log( "\t"+'# of jQuery(this).find(.mtli_attachment) : '+jQuery(this).find('.mtli_attachment').length );
//console.log( "\t"+'# of jQuery(this).find([rel^="mtli_filesize"]) : '+jQuery(this).find('[rel^="mtli_filesize"]').length );
